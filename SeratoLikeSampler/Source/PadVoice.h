...
struct PadSlice { int startSample = 0; int endSample = 0; int midiNote = 36; float gainLin { 1.0f }; };
...
public:
    ...
    void startNote (const juce::AudioBuffer<float>& src, const PadSlice& slice) {
        source = &src; current = slice; pos = current.startSample; adsr.noteOn(); active = true; sliceGainLin = current.gainLin;
    }
    ...
    void render (juce::AudioBuffer<float>& out, int startSample, int numSamples) {
        ...
        const int sliceRemaining = current.endSample - pos;
        const int toCopy = juce::jlimit (0, numSamples, sliceRemaining);
        if (toCopy > 0) {
            for (int ch = 0; ch < temp.getNumChannels(); ++ch)
                temp.copyFrom (ch, 0, *source, juce::jmin (ch, source->getNumChannels()-1), pos, toCopy);
            pos += toCopy;
        }
        ...
        if (pos >= current.endSample || ! adsr.isActive()) active = false;
    }
private:
